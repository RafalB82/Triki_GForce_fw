...
| P0.09 | nieuzywany (INT1 ukladu niepolaczony) | — |
| P0.10 | **LSM INT2/DRDY** (pin 9 ukladu) — DRDY_XL przez INT2_CTRL (0x0E), GPIOTE, timestamp probek | [P] plyta 2026-08-30 + datasheet ukladu |
...
| Model | gravity estimator (filtr komplementarny): propagacja gyro `dg = -(w x g)*dt` co ramke (q16.16 x q8.8, zaokraglana), **na gyro skorygowanym o nauczony bias** (0.3.7); korekcja ACC gated (**|w-wbias| < 15 dps** — gate NIE moze byc ponizej spec biasu gyro ±5 dps; 0.3.4 lekcja) I innowacja < 1.0 m/s^2 + powolny leak 1/2048 zawsze (net dead-lockow), renormalizacja do g; **nauka biasu gyro w quasi-bezruchu** (||lin|| < 1.2 m/s^2, tau 0.6s — 0.3.7: bez tego dryf biasu podtrzymywal blad g na progu gate'a => raczeta velocity przy ruchach, log wire v2 21:42); `lin = LPF(acc - g_est)` (JEDNO LPF na roznicy); `a_move = dot(lin, axis)`; `v += a_move*dt`; os ruchu default X barbell (vbt_set_axis) |
...
7. INT1/DRDY — **ZAMKNIETE 0.3.8 [P]**: DRDY = INT2 ukladu (pin 9) -> **P0.10** nRF. Trzy nakladajace sie przyczyny braku DRDY w 0.3.1-0.3.6: (a) zly register INT1_CTRL zamiast INT2_CTRL (0.3.5 fix), (b) probe: FIFO timestampow 4 sloty < prog 5 krawedzi => nigdy nie przechodzila (0.3.8 fix — dedykowany licznik), (c) pin P0.10 potwierdzony plyta (0.3.6). Weryfikacja: RTT `S2 DRDY ok pin=P0.10 pol=y`, tempo seq w wire v2 = 104/s (polling daje 111/s).
