...
| **v0.4.1_rtt** | fix smoke 0.4.0: runtime reg access (inactivity + WAKE_UP_SRC) przez **reg_write_t/reg_read_t** (TWIM jak zyje, fault → uninit + fallback bb, ban po 3; wzorzec read_motion) — reg_write/reg_read (bb) po `nrfx_twim_enable` przejmujacym piny 5/6 pisaly w pustke; sync readback przy CONNECTED przeniesiony ze SWI (BLE handler) do main loop (`g_conn_sync_req`); boot-init bez zmian (bb przed TWIM = stuck-bus recovery); **[P] ZWALIDOWANY NA SPRZĘCIE (log nRF Connect 2026-09-01 13:14)**: inact cfg=E1/ths=01/dur=04/md1=80 z readbackiem, tempo ACTIVE 103.9/s → **IDLE 13.0/s** (12.5Hz HW LP, seq ciągły) → powrót 103.7/s po ruchu (**auto-restore ODR potwierdzony** — DS6207 6.5.2), conn params 15ms/lat0 ↔ 150ms/lat4 przejścia w rytm S6, sesja 70s/5159 ramek bez drop; harness 7/7; dev text=35024 data=144 bss=3908, hex `firmware/hex/trikig_triki_v0.4.1_rtt.hex` sha256 `9f5c775f…` — **[P] smoke RTT: IDLE cykl działa (S6 in/out, tempo 104↔12.5), ale dtf +1/próbkę w IDLE + cpfail +1/przejście** → fix 0.4.2 |
| **v0.4.3_rtt** | fix smoke 0.4.2: velocity w IDLE = 0 BY DESIGN (`trikig_vbt: s_idle => s_vel=0`) — klasa ruchu 30-250mg integrowała phantom velocity @12.5Hz z zamrożonym gyro aż do clampa 15625 mm/s (log smoke: v=12593→15625); harness 7/7; dev text=35088 bss=3908, hex `firmware/hex/trikig_triki_v0.4.3_rtt.hex` sha256 `8407b130…` — **[P] boot/smoke 2026-09-03: czysty (inact E1/01/04/80, S6 in→IDLE po 4s); trening dip/pullup [P]: 21-36% pokrycia ramek (HW śpi w serii — klasa 140-246mg p95 < WK_THS 250mg) → fix 0.5.0** |
| **v0.5.0_rtt** (training mode, MINOR — nowa komenda RX, D-020) | (1) **RX `20 18 01/00` = training mode** — ON: `lsm6dsl_inactivity_enable(false)` + readback `WAKE_UP_SRC`, sleeping ⇒ `lsm6dsl_wake_force()` (jawne CTRL1_XL/CTRL2_G=V19 + readback; INACT_EN=00 nie jest w DS6207 opisane jako wybudzenie) ⇒ `idle_connected_set(false)` + `vbt_reset()` (czysta kalibracja); OFF: HW sam zejdzie w IDLE po SLEEP_DUR; **auto-OFF przy disconnect** (safety); RX handler tylko flaguje (`g_train_req`), I2C w main (wzorzec `g_conn_sync_req`); diag: RTT `S6 train ON/OFF`, DIAG `train=`; motywacja [P] trening 2026-09-03: klasa dip/pullup 140-246mg p95 pod WK_THS 250mg (podłoga LSB @FS16g — strojenie progu odpada) (SPEC 6.2, 11.14j); (2) SLEEP_DUR 4s→**6s** (margines na starty serii bez komendy); WK_THS bez zmian; harness 7/7 (flash/smoke PENDING) |
| **v0.4.2_rtt** | fix smoke 0.4.1: (1) **RTC1 = 16384Hz** (app_timer2 PRESCALER=1, 61.035µs/tick — nie 30.5µs); progi IDLE przeliczone (min 655/max 1966/wdt 2621/gap 3277), dt q16.16 = ticks×4; odkrycie uboczne: stary DT_GAP_TICKS 1966 = de facto **120ms** nie 60ms (ACTIVE nieczuły — zostaje, komentarz poprawiony); (2) cpfail +1/przejście: readback WAKE_UP_SRC kasuje LIR → TOGGLE łapie deassert → drugi event z tym samym stanem → change_conn_params BUSY; idle_cp_apply gated na zmianę stanu; obserwacje [P]: ruch powoli <250mg NIE wybudza z IDLE (v rampuje @12.5Hz, ZUPT po rest — WK_THS do strojenia), wake burst bdrop +~80 przejściowo; harness 7/7; dev text=35088 bss=3908, hex `firmware/hex/trikig_triki_v0.4.2_rtt.hex` sha256 `652be74f…` — flash/smoke PENDING |
